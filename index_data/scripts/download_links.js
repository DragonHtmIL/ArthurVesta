function linkDownAl() {
  window.location.href="index_data/documents/AV_Azurland_1.20.1.jar";
};
function linkDownBdom() {
  window.location.href="index_data/documents/AV_Better_Discs_of_Minecraft_1.20.1.jar";
};
function linkDownGom() {
  window.location.href="index_data/documents/AV_Gems_of_ Minecraft_1.20.1.jar";
};
function linkDownAse() {
  window.location.href="index_data/documents/Appelskin's Earth.zip";
};
function linkDownEpS24u() {
  window.location.href="index_data/documents/Eyephone S24 Ultra§0.zip";
};
function linkDownMcIl() {
  window.location.href="index_data/documents/Minecraft is Latest§0.zip";
};
function linkDownBsmc() {
  window.location.href="index_data/documents/For Block Story 13.3.3.zip";
};